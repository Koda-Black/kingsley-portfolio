...
const roles = [
  { period: "Present", title: "Co-founder", company: "AERA", copy: "Building globally significant technology companies alongside exceptional African founders, from talent discovery through launch and scale." },
  { period: "Present", title: "Co-founder", company: "Ajo Cash", copy: "Building digital trust infrastructure for Nigerian rotating savings groups with automated contributions, instant payouts and accountable community finance." },
...
export default function ExperiencePage() {
  return <PageFrame eyebrow="03 / Experience" title="Built with teams. Measured by impact." lede="Technical leadership, fullstack execution and product-minded engineering."><div className="timeline inner-timeline">{roles.map((role) => <article key={role.company}><span>{role.period}</span><h3>{role.title}</h3><strong>{role.company}</strong><p>{role.copy}</p></article>)}</div></PageFrame>;
}
